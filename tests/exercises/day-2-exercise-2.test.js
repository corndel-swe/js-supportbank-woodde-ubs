...
describe("validateId", () => {
    ...
    it("should throw an error if the id isn't defined", () => {
        ...
        assert.throws(() => validateId(invalidId), 'Expected an error to be thrown for an undefined id')
    })

    it("should throw an error if the id isn't a string", () => {
        const invalidId = 1
        assert.throws(() => validateId(invalidId), 'Expected an error to be thrown for a non-string id')
    })

    it("should throw an error if the id isn't an odd number of characters long", () => {
        const invalidId = 'abac'
        assert.throws(() => validateId(invalidId), 'Expected an error to be thrown for an id with an even number of characters')
    })

    it("should throw an error if the id doesn't contain the letter 'a'", () => {
        const invalidId = 'bbc'
        assert.throws(() => validateId(invalidId), 'Expected an error to be thrown for an id without the letter "a"')
    })

    it("should throw an error if the id isn't all lowercase", () => {
        const invalidId = 'aBaci'
        assert.throws(() => validateId(invalidId), 'Expected an error to be thrown for an id with uppercase characters')
    })
})
